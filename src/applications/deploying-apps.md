...
!!! Known Issue
		...
		specifically manifest itself if the controller crashes while you are scaling an app. The scale
		operation will continue, but the controller will not start back up properly until the operation
		is complete. We intend to fix this issue soon and are tracking the work in
